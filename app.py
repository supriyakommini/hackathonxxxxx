...
Phase 3 (AI): Grok-powered risk explanations, sentiment badges, and retention action plans.
"""
...
# Load .env from project root so GROK_SECRET_KEY is available at module level
...
_GROK_MODEL    = "llama-3.3-70b-versatile"  # fast + free-tier on Groq


def _grok_client() -> OpenAI | None:
    """Return an OpenAI-compatible client pointed at Grok, or None if key missing."""
    key = os.getenv("GROK_SECRET_KEY")
    if not key:
        return None
    return OpenAI(api_key=key, base_url=_GROK_BASE_URL)


def _call_grok(prompt: str, system: str = "You are a concise HR analytics assistant.",
               max_tokens: int = 300) -> str:
    """
    Call Grok with a single user prompt. Returns the text response.
    Raises RuntimeError if the key is not configured.
    """
    client = _grok_client()
    if client is None:
        raise RuntimeError("GROK_SECRET_KEY not found in environment / .env file.")
...
def inject_css() -> None:
    ...
    st.markdown(
        ...
        /* ── Sidebar AI Assistant toggle button ── */
        ...
        [data-testid="stSidebar"] .stButton > button:hover {{
            background-color: #EA6B0A !important;
            color: {WHITE} !important;
        }}

        /* ── Chat column isolation backdrop ── */
        ...
        .chat-input-row .stButton > button:hover {{
...
def page_risk_table(df: pd.DataFrame) -> None:
    ...
    event = st.dataframe(
        ...
        column_config={
            "Prob %": st.column_config.ProgressColumn(
                "Prob %", format="%.1f%%", min_value=0, max_value=1,
            ),
    ...
    # Right column — proxy risk drivers, AI narrative, sentiment + feedback
    with right:
        st.markdown(
            f"<div style='font-size:0.78rem;font-weight:700;color:{GREY};"
        ...
        # ── Feature 1: AI risk explanation (on-demand button) ─────────────────
        exp_key = f"ai_risk_exp_{emp_row['Employee_ID']}"
        if exp_key not in st.session_state:
            st.session_state[exp_key] = None

        if st.button("Generate AI Explanation", key=f"btn_exp_{emp_row['Employee_ID']}",
                     use_container_width=True):
            pop_medians = {c: df[c].median() for c in
                ["Job_Satisfaction", "Work_Life_Balance", "Management_Support",
                 "Career_Development", "Engagement_Level"]}
            # Find the survey dimension furthest below median
            deltas = {
                c: emp_row[c] - v
                for c, v in pop_medians.items()
            }
            worst_dim = min(deltas, key=deltas.get)
            worst_gap = deltas[worst_dim]
            compa = emp_row.get("Compa_Ratio", 1.0)
            compa_note = (
                f"paid {(1-compa)*100:.0f}% below market" if compa < 0.95
                else f"paid {(compa-1)*100:.0f}% above market" if compa > 1.05
                else "paid at market rate"
            )
            prompt = (
                f"Employee profile: {emp_row['Role']}, {emp_row['Department']}, "
                f"{emp_row['Tenure_Years']:.1f} years tenure, {emp_row['Employment_Type']}.\n"
                f"Model output: {tier} risk tier, {prob:.1%} attrition probability.\n"
                f"Compensation: {compa_note} (compa-ratio {compa:.2f}).\n"
                f"Biggest survey gap vs workforce median: "
                f"{worst_dim.replace('_', ' ')} is {emp_row[worst_dim]:.1f} "
                f"vs median {pop_medians[worst_dim]:.1f} ({worst_gap:+.1f}).\n"
                f"All survey scores vs median: "
                + ", ".join(
                    f"{c.replace('_', ' ')} {emp_row[c]:.1f} (med {v:.1f})"
                    for c, v in pop_medians.items()
                )
                + "\n\nIn exactly 2 sentences for a line manager: "
                "(1) name the single most important risk factor with its specific number, "
                "(2) state the most likely consequence if unaddressed. "
                "Be direct. No hedging. Do not start with 'This employee'."
            )
            with st.spinner("Generating explanation..."):
                try:
                    st.session_state[exp_key] = _call_grok(
                        prompt,
                        system="You are a direct, data-driven HR analytics assistant. Give specific, evidence-based observations. Never use filler phrases.",
                        max_tokens=180,
                    )
                except Exception as e:
                    st.session_state[exp_key] = f"Error: {e}"

        if st.session_state[exp_key]:
            st.markdown(
                f"<div style='background:#FFF7ED;border-left:3px solid {ORANGE};"
                f"border-radius:0 6px 6px 0;padding:10px 12px;font-size:0.83rem;"
                f"color:{BLACK};line-height:1.55;margin-top:4px'>"
                f"{st.session_state[exp_key]}</div>",
                unsafe_allow_html=True,
            )

        st.markdown("<br>", unsafe_allow_html=True)

        # ── Feature 2: Sentiment badge (auto on row select) ───────────────────
        feedback = str(emp_row.get("Feedback_Comments", "")).strip()
        if feedback and feedback.lower() not in ("nan", ""):
            sent_key = f"ai_sentiment_{emp_row['Employee_ID']}"
            if sent_key not in st.session_state:
                try:
                    sent_prompt = (
                        f"Classify this employee feedback. "
                        f"Reply with ONLY one word on the first line: Positive, Neutral, or Negative. "
                    ...
                    raw_sent = _call_grok(
                        ...
                        max_tokens=5,
                    ).strip().split()[0].capitalize()
                    label = raw_sent if raw_sent in ("Positive", "Neutral", "Negative") else "Neutral"
                    st.session_state[sent_key] = {"label": label}
                except Exception:
                    st.session_state[sent_key] = {"label": "Neutral"}

            sent_data  = st.session_state[sent_key]
            sent_label = sent_data["label"]
            sent_color = {"Positive": "#16A34A", "Negative": RED, "Neutral": GREY}.get(sent_label, GREY)
            sent_bg    = {"Positive": "#F0FDF4", "Negative": "#FEF2F2", "Neutral": GREY_LIGHT}.get(sent_label, GREY_LIGHT)

            st.markdown(
                f"<div style='font-size:0.78rem;font-weight:700;color:{GREY};"
                f"text-transform:uppercase;letter-spacing:0.06em;margin-bottom:0.4rem'>"
                "Feedback Comment</div>"
                f"<div style='display:inline-block;background:{sent_color};color:{WHITE};"
                f"border-radius:4px;padding:1px 8px;font-size:0.72rem;font-weight:700;"
                f"margin-bottom:6px'>{sent_label}</div>"
                f"<div style='font-style:italic;font-size:0.85rem;color:{BLACK};"
                f"background:{sent_bg};padding:10px 12px;border-radius:6px;"
                f"border-left:3px solid {sent_color}'>&#x201C;{feedback}&#x201D;</div>",
                unsafe_allow_html=True,
            )
...
def page_retention_planner(df: pd.DataFrame, matrix: pd.DataFrame) -> None:
    ...
    # ── AI Intervention Plan (uses live calculator state) ─────────────────────
    st.markdown("<br>", unsafe_allow_html=True)
    calc_key = f"ai_calc_plan_{sel_persona}_{sel_tier_c}_{reduction_pct}_{cost_per_emp}"
    # Pull primary driver for selected persona from matrix
    driver_info = ""
    if sel_persona != "All personas":
        p_match = matrix_aug[matrix_aug["Persona"] == sel_persona]
        if not p_match.empty:
            driver_info = (
                f"Primary attrition driver for {sel_persona}: {p_match.iloc[0]['Primary Driver']}. "
                f"Existing handbook recommendation: {p_match.iloc[0]['Recommended Action']}."
            )
    else:
        top_drivers = (
            matrix_aug.sort_values("high_risk_n", ascending=False)
            .head(2)["Primary Driver"]
            .tolist()
        )
        driver_info = f"Top attrition drivers across personas: {', '.join(top_drivers)}."

    btn_label = "Regenerate Intervention Plan" if st.session_state.get(calc_key) else "Generate Intervention Plan"
    if st.button(btn_label, key="btn_ai_calc_plan", use_container_width=False):
        roi_sign  = "+" if roi_pct >= 0 else ""
        prompt = (
            f"You are an HR strategy consultant advising a CHRO. "
            f"The following retention intervention has been scoped in our attrition risk tool.\n\n"
            f"SCOPE\n"
            f"  Target group: {sel_persona}, {sel_tier_c} risk"
            f" ({n_target:,} employees)\n"
            f"  {driver_info}\n\n"
            f"FINANCIALS\n"
            f"  Baseline attrition cost: ${baseline_cost/1_000_000:.2f}M\n"
            f"  Target attrition reduction: {reduction_pct}%\n"
            f"  Projected saving if target met: ${saving/1_000_000:.2f}M\n"
            f"  Programme budget: ${prog_cost/1_000_000:.2f}M "
            f"(${cost_per_emp:,} per employee)\n"
            f"  Net saving: ${net_saving/1_000_000:.2f}M  |  ROI: {roi_sign}{roi_pct:.0f}%\n\n"
            f"Write a sharp, specific intervention plan to achieve the {reduction_pct}% "
            f"attrition reduction target within this budget. Structure as:\n"
            f"OBJECTIVE — one sentence stating the goal in business terms\n"
            f"Step 1 [Owner] — Specific action (Timeline)\n"
            f"Step 2 [Owner] — Specific action (Timeline)\n"
            f"Step 3 [Owner] — Specific action (Timeline)\n"
            f"RISK — one sentence on the biggest execution risk\n"
            f"SUCCESS METRIC — one concrete, measurable KPI to track progress\n\n"
            f"Address the primary driver directly. Be specific to this budget and group size. "
            f"No generic advice."
        )
        with st.spinner("Generating intervention plan..."):
            try:
                st.session_state[calc_key] = _call_grok(
                    prompt,
                    system="You are a senior HR strategy consultant. Be specific, direct, and data-driven. Never use filler phrases.",
                    max_tokens=420,
                )
            except Exception as e:
                st.session_state[calc_key] = f"Error: {e}"

    if st.session_state.get(calc_key):
        net_color_plan = "#22C55E" if net_saving >= 0 else RED
        st.markdown(
            f"<div style='background:{WHITE};border-radius:8px;"
            f"border-left:4px solid {net_color_plan};"
            f"box-shadow:0 2px 8px rgba(0,0,0,0.08);"
            f"padding:1.1rem 1.4rem;margin-top:0.5rem'>"
            f"<div style='font-size:0.72rem;font-weight:700;color:{net_color_plan};"
            f"text-transform:uppercase;letter-spacing:0.07em;margin-bottom:0.6rem'>"
            f"AI Intervention Plan — {sel_persona} / {sel_tier_c} / {reduction_pct}% reduction target</div>"
            f"<div style='font-size:0.84rem;color:{BLACK};white-space:pre-line;line-height:1.75'>"
            f"{st.session_state[calc_key]}</div></div>",
            unsafe_allow_html=True,
        )
        ...
        st.dataframe(
            ...
            column_config={
                "Attrition Prob": st.column_config.ProgressColumn(
                    "Attrition Prob", format="%.1f%%", min_value=0, max_value=1,
                ),
                "Est. Cost": st.column_config.NumberColumn(
                    "Est. Cost", format="$%,.0f",
                ),
...
_CHAT_SUGGESTIONS = [
    ...
    "What are the top drivers of attrition in this dataset?",
    "Suggest retention strategies for high-risk employees.",
    "How should I interpret the compa-ratio in this context?",
    "What does a High risk tier mean for an employee?",
]


def render_chat_panel(df: pd.DataFrame, winner: dict) -> None:
    """Renders the AI assistant chat panel inside whatever column it is called from."""
    ctx        = _build_chat_context(df, winner)
    high_count = (df["risk_tier"] == "High").sum()

    # ── Inject column background as a scoped style ────────────────────────────
    # Targets the last Streamlit column in the top-level horizontal block,
    # which is always the chat column when the panel is open.
    st.markdown(
        """<style>
    ...
    st.markdown(
        ...
        f"<div class='chat-panel-header'>"
        f"<div class='chat-panel-header-title'>"
        f"<span class='chat-panel-dot'></span>HR AI Assistant</div>"
        f"<div class='chat-panel-sub'>"
        f"Llama 3.3 70B &nbsp;&#8226;&nbsp; "
        f"{len(df):,} employees &nbsp;&#8226;&nbsp; "
        f"{high_count} high-risk</div>"
        f"</div></div>",
        unsafe_allow_html=True,
    )
    ...
    if not chat_history:
        st.markdown(
            ...
            "Ask about attrition risk, model results, or retention strategies."
            "</div>",
            unsafe_allow_html=True,
        )
        for i, suggestion in enumerate(_CHAT_SUGGESTIONS):
            if st.button(suggestion, key=f"btn_sugg_{i}", use_container_width=True):
                st.session_state["_queued_chat"] = suggestion
                st.rerun()
        st.markdown("<hr class='chat-divider'>", unsafe_allow_html=True)

    # ── Render history as a single scrollable HTML block ──────────────────────
    # All bubbles are built as one string so they share the scroll container.
    else:
        bubbles_html = "<div class='chat-messages-scroll'>"
        for msg in chat_history:
            if msg["role"] == "user":
                # Plain-escape user input — no markdown interpretation needed
                ...
                # Convert markdown from AI responses to HTML
                content = _md_to_html(msg["content"])
                bubbles_html += (
                    f"<div class='chat-role-label'>AI Assistant</div>"
    ...
    # ── Input row (form keeps submit from full page rerun on Enter) ───────────
    queued = st.session_state.pop("_queued_chat", None)
    if queued:
        _handle_chat_message(queued, chat_history, ctx)
        st.rerun()
    ...
    with st.form(key="chat_form", clear_on_submit=True, border=False):
        ...
        with input_col:
            user_text = st.text_input(
                ...
                placeholder="Ask about attrition…"
            )
        with send_col:
    ...
    if submitted and user_text.strip():
        _handle_chat_message(user_text.strip(), chat_history, ctx)
        st.rerun()

    # ── Clear link ────────────────────────────────────────────────────────────
    if chat_history:
        if st.button("Clear conversation", key="btn_clear_chat_panel",
                     use_container_width=True):
            st.session_state["chat_history"] = []
            st.rerun()
...
def _handle_chat_message(
    user_input: str, chat_history: list, ctx: str
) -> None:
    """Append user message, call Groq, append reply — all without rendering."""
    chat_history.append({"role": "user", "content": user_input})

    system_prompt = (
        "You are a concise HR analytics assistant embedded in an attrition risk dashboard. "
        "Answer questions about employee attrition, retention strategies, and model outputs. "
        "Be direct and specific. Keep answers under 180 words unless asked to elaborate. "
    ...
    try:
        client = _grok_client()
        if client is None:
            reply = "AI unavailable: GROK_SECRET_KEY not configured."
        else:
...
def _md_to_html(text: str) -> str:
    """Convert common markdown patterns to safe HTML for chat bubble rendering.
    Escapes HTML first so AI-generated angle brackets cannot inject markup."""
    safe = _html.escape(text)
    # Bold must be converted before italic to avoid consuming `*` pairs twice
...
def _build_chat_context(df: pd.DataFrame, winner: dict) -> str:
    """Build a compact workforce summary to ground the chatbot's answers."""
    total   = len(df)
    high    = (df["risk_tier"] == "High").sum()
    medium  = (df["risk_tier"] == "Medium").sum()
    low     = (df["risk_tier"] == "Low").sum()
    top_dept = (
        df[df["risk_tier"] == "High"]["Department"]
        .value_counts()
        .idxmax()
    ) if high > 0 else "N/A"
    avg_prob = df["attrition_probability"].mean()
    model_name = winner.get("winner_model", "XGBoost")
    roc = winner.get("roc_auc", "N/A")
    return (
        f"Workforce snapshot: {total} employees scored for attrition risk. "
        f"Risk tiers — High: {high} ({high/total:.0%}), "
        f"Medium: {medium} ({medium/total:.0%}), "
        f"Low: {low} ({low/total:.0%}). "
        f"Average attrition probability: {avg_prob:.1%}. "
        f"Department with most high-risk employees: {top_dept}. "
        f"Winning model: {model_name} (ROC-AUC {roc}). "
        "The user is an HR professional reviewing this dashboard."
    )
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        # Chat toggle button
        toggle_label = "Close AI Assistant" if chat_open else "AI Assistant"
        if st.button(toggle_label, key="btn_chat_toggle", use_container_width=True):
    ...
    # ── Layout: split when chat panel is open ─────────────────────────────────
    if chat_open:
        content_col, chat_col = st.columns([1.7, 1], gap="small")
    else:
